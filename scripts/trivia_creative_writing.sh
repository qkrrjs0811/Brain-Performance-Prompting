...
# 여러 모델을 한 번의 스크립트 실행으로 순차 실험: 앞 모델의 모든 METHOD×RUN이 끝난 뒤 다음 모델. MODELS가 비우면 MODEL만 사용합니다.
MODELS=(
  "gpt-5.4"
)
# 선택: MODELS_RESOLVED 길이와 같게 두면 모델 순서와 1:1로 CUDA_VISIBLE_DEVICES를 붙입니다.
OPEN_MODEL_CUDA_DEVICES=(0)
...
DATA_FILE="${DATA_FILE:-trivia_creative_writing_100_n_5.jsonl}" # dataset: trivia_creative_writing_100_n_5.jsonl, trivia_creative_writing_100_n_10.jsonl
START_IDX=0
END_IDX=100
...
# RUNS=3 RUN_START=1 USE_BATCH=0 bash scripts/trivia_creative_writing.sh
# 추가로, --additional_output_note를 넘기면 run prefix로 "기본값 _runXX" 형태가 됩니다.
RUNS="${RUNS:-1}"
RUN_START="${RUN_START:-7}"
RUN_WIDTH="${RUN_WIDTH:-2}" # 2자리: run01, run02 ...
# open_model(vLLM) replicate마다 SamplingParams.seed = SEED_BASE + RUN (RUN은 RUN_START..)
SEED_BASE="${SEED_BASE:-42}"
USE_BATCH="${USE_BATCH:-1}" # 1이면 run_batch.py 사용
GPU_MEMORY_UTILIZATION="${GPU_MEMORY_UTILIZATION:-0.95}" # open_model(vLLM)일 때만 사용
