                    ...
                    if "answer_1" in data:
                        ...
                        if "test_output_infos" in value:
                            for test_info in value["test_output_infos"]:
                                correct = test_info.get("correct", False)
                                accuracy = 1 if correct else 0
                                accuracies.append(accuracy)
                except json.JSONDecodeError:
                    print(
                        f"JSONDecodeError at line {line_number}: Line could not be parsed. Skipping this line."
                    )
...
def get_root_dir_by_model(model_name):
    base_dir = "logs/hf_multiple_choice"
    ...
    if model_name == "gpt-4o":
        ...
        return {
            "with_sys_mes": os.path.join(
                base_dir, "meta-llama-Llama-3.1-8B-Instruct_w_sys_mes"
            ),
            "wo_sys_mes": os.path.join(
                base_dir, "meta-llama-Llama-3.1-8B-Instruct_wo_sys_mes"
            ),
        }
...
def parse_args():
    p = argparse.ArgumentParser(description="HF multiple-choice 로그에서 정확도 집계")
    p.add_argument("--model", type=str, choices=SUPPORTED_MODELS, default="gpt-5.4-nano")
    p.add_argument(
        "--reasoning_effort",
        type=str,
        default="none",
...
if __name__ == "__main__":
    ...
    if os.path.exists(with_dir):
        process_all_files(
            with_dir,
            f"accuracy_results_{selected_model}_with_sys_mes.xlsx",
        )
    else:
    ...
    if os.path.exists(wo_dir):
        process_all_files(
            wo_dir,
            f"accuracy_results_{selected_model}_wo_sys_mes.xlsx",
        )
    else:
