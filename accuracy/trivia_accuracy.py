...
def calculate_accuracy(file_path):
    ...
    for i in range(len(result)):
        for data in result[i].get('test_output_infos', []):
            accuracy = data['correct_count'] / data['question_count']
                    ...
                    if 'answer_1' in data:
                        ...
                        if 'test_output_infos' in value:
                            for test_info in value['test_output_infos']:
                                correct_count = test_info.get('correct_count', 0)
                                question_count = test_info.get('question_count', 0)
                                if question_count > 0:
...
def get_root_dir_by_model(model_name):
    ...
    if model_name == 'gpt-4o':
        ...
        # run.py: model id의 "/" -> "-" 가 폴더명 (Qwen/Qwen2.5-7B-Instruct)
        return {
...
def parse_args():
    p = argparse.ArgumentParser(description="Trivia creative writing 로그에서 정확도 집계")
    p.add_argument(
        "--model",
        type=str,
